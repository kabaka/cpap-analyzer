...
export class DashboardPage extends BasePage {
  ...
  async openSession(date: string) {
    await this.sessionTable.getByRole('row', { name: new RegExp(date) }).click();
  }
...
export class ImportWizardPage extends BasePage {
  ...
  async waitForImportComplete(timeout = 60000) {
    await expect(this.page.getByRole('heading', { name: /import complete/i })).toBeVisible({
      timeout,
    });
  }
...
export async function generateSyntheticEDF(options: EDFGeneratorOptions): Promise<Blob> {
...
test.describe('Data Import', () => {
  ...
  test('should display progress during import', async ({ page, importWizardPage }) => {
    await importWizardPage.goto();
  ...
  test('should handle malformed files gracefully', async ({ page, importWizardPage }) => {
    await importWizardPage.goto();
  ...
  test('should display warnings for corrupted files', async ({ importWizardPage }) => {
    await importWizardPage.goto();
...
test.describe('Statistical Analysis', () => {
  ...
  test('should handle insufficient data gracefully', async ({ page, analysisPage }) => {
    await analysisPage.goto('statistical');
...
test.describe('Chart Interactions', () => {
  ...
  test('should zoom chart with mouse wheel', async ({ page, sessionDetailPage }) => {
  ...
  test('should synchronize zoom across multiple charts', async ({ page, sessionDetailPage }) => {
    await sessionDetailPage.goto('session-1');
    await sessionDetailPage.openSignalViewer();
  ...
  test('should reset zoom with reset button', async ({ page, sessionDetailPage }) => {
    await sessionDetailPage.goto('session-1');
    await sessionDetailPage.openSignalViewer();
...
test.describe('Report Export', () => {
  ...
  test('should include selected date range in export', async ({ page, dashboardPage }) => {
    await dashboardPage.goto();
...
test.describe('Settings Persistence', () => {
  ...
  test('should persist date range selection', async ({ page, dashboardPage, importedData }) => {
    await dashboardPage.goto();
    await dashboardPage.selectDateRange('Last 7 Days');
    ...
    const selectedRange = await page.getByTestId('date-range-selector').textContent();
...
test.describe('Malformed Data Handling', () => {
  test('should reject file with invalid EDF header', async ({ page, importWizardPage }) => {
    await importWizardPage.goto();
    await importWizardPage.uploadFile('tests/fixtures/edf/malformed-header.edf');

    await expect(page.getByRole('alert')).toContainText(/invalid EDF header/i);

    // Should not crash
    await expect(page.getByRole('button', { name: /try again/i })).toBeVisible();
  });

  test('should warn about corrupted data but continue', async ({ page, importWizardPage }) => {
    await importWizardPage.goto();
    await importWizardPage.uploadFile('tests/fixtures/edf/corrupted-data.edf');
...
```typescript
test('should handle 1+ year of data (large file)', async ({ page, importWizardPage }) => {
...
test('should correctly split multi-day sessions', async ({
  ...
  const sessionDate = await page.getByRole('row').first().getByRole('cell').first().textContent();
...
test('should load custom analysis plugin', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Test Analysis' })).toBeVisible();
});
```
...
test.describe('Keyboard Navigation', () => {
  ...
  test('should navigate table with arrow keys', async ({ page, dashboardPage, importedData }) => {
    await dashboardPage.goto();
...
test.describe('ARIA Validation', () => {
  ...
  test('should have valid ARIA roles', async ({ page }) => {
    ...
    const invalidRoles = await page
      .locator(
        '[role]:not([role="banner"],[role="navigation"],[role="main"],[role="contentinfo"],[role="complementary"],[role="search"],[role="form"],[role="dialog"],[role="alertdialog"],[role="alert"],[role="status"],[role="table"],[role="row"],[role="cell"],[role="columnheader"],[role="rowheader"],[role="button"],[role="link"],[role="tab"],[role="tablist"],[role="tabpanel"],[role="region"],[role="img"],[role="figure"])',
      )
      .count();
...
test.describe('Color Contrast', () => {
  test('should meet WCAG AA contrast ratios in light theme', async ({ page }) => {
    ...
    const contrastRatio = calculateContrastRatio(color.foreground, color.background);
...
test.describe('Interaction Responsiveness', () => {
  ...
  test('should respond to button clicks within 100ms', async ({ page, dashboardPage }) => {
    await dashboardPage.goto();
  ...
  test('should filter session table within 200ms', async ({ page, dashboardPage }) => {
    await dashboardPage.goto();
...
test.describe('Large Dataset Performance', () => {
  ...
  test('should import 1 year of data within 2 minutes', async ({ page, importWizardPage }) => {
    await importWizardPage.goto();
  ...
  test('should maintain 60fps during chart pan', async ({ page, sessionDetailPage }) => {
...
test.describe('Memory Leak Detection', () => {
  test('should not leak memory on repeated navigation', async ({ page, importedData }) => {
    await page.goto('/');
  ...
  test('should clean up chart event listeners', async ({ page, sessionDetailPage }) => {
    await sessionDetailPage.goto('session-1');
    await sessionDetailPage.openSignalViewer();
...
```typescript
test('should handle synthetically generated session', async ({ page, importWizardPage }) => {
