...
class ResMedPlugin implements MachinePlugin {
  ...
  detect(files: FileMetadata[]): boolean {
    ...
    const hasResMedFiles = files.some((f) => /STR\.edf|BRP\.edf|EVE\.edf/i.test(f.name));
...
test('import ResMed SD card data', async ({ page }) => {
  ...
  await fileInput.setInputFiles(['./test-data/DATALOG/BRP.edf', './test-data/DATALOG/EVE.edf']);
