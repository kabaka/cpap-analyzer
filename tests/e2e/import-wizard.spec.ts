 ...
 *      BOTH sessions are created — via the wizard's "Sessions imported" count
 *      AND the Sessions list/count UI — with ZERO errors.
 *
 ...
 *      block. It must NOT appear as an error; instead the wizard surfaces an
 *      "Empty files skipped" count (≥1) via `ImportProgress.filesSkippedEmpty`.
 *
...
/**
 * Read the numeric value rendered next to a summary label in the wizard's
 * completion grid. Returns `null` when that summary item is absent (e.g. the
 * "Errors" item only renders when `progress.errors.length > 0`).
 */
async function summaryValueFor(page: Page, label: string): Promise<string | null> {
  const item = page
    .locator('[class*="summaryItem"]')
    .filter({ has: page.getByText(label, { exact: true }) });
  if ((await item.count()) === 0) return null;
  // The value span precedes the label span within the same summary item.
  return (await item.locator('[class*="summaryValue"]').first().textContent())?.trim() ?? null;
}
...
async function runImport(page: Page): Promise<void> {
  ...
  // The wizard auto-advances through scan → import → complete. Allow generous
  // time for worker-pool spin-up + parse on slower CI machines.
...
test.describe('Import Wizard — full directory import', () => {
  ...
  test('imports a multi-session day and skips the empty CSL stub with zero errors', async ({
    ...
    // machineId_date uniqueness constraint. We assert the POSITIVE count here,
    // not merely the absence of an error.
    expect(await summaryValueFor(page, 'Sessions imported')).toBe(String(tree.expectedSessions));

    // ── The CSL stub is reflected in the empty-skip count, NOT the errors. ──
    // Regression #2: ImportProgress.filesSkippedEmpty surfaced as "Empty files
    // skipped". The item only renders when the count is > 0, so its presence is
    // itself meaningful.
    const skipped = await summaryValueFor(page, 'Empty files skipped');
    expect(skipped).not.toBeNull();
    expect(Number(skipped)).toBeGreaterThanOrEqual(tree.expectedSkippedEmpty);

    // ── ZERO errors surfaced. ──
    // No "Errors" summary item, and no expandable file-errors section.
    expect(await summaryValueFor(page, 'Errors')).toBeNull();
    await expect(page.getByRole('button', { name: /file errors/i })).toHaveCount(0);
